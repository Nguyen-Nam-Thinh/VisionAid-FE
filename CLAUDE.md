...
- Được phép tự tạo nhánh công việc, commit, push tiến độ và tích hợp phần đã kiểm tra vào `dev` theo quy trình Git bên dưới, không cần hỏi lại cho từng lần. Không tự reset, force-push, merge vào `main`/`master` hoặc tạo PR nếu người dùng chưa yêu cầu. Không sửa repository BE/MO khi nhiệm vụ chỉ thuộc FE.
...
- Khi bắt đầu nhiệm vụ có sửa file, kiểm tra branch, working tree và remote, fetch để xác định trạng thái mới nhất của `dev`. Tạo nhánh riêng từ `dev` đã cập nhật trước khi sửa; nếu đang tiếp tục đúng nhiệm vụ trên nhánh riêng thì dùng lại nhánh đó. Nếu remote chưa có `dev`, tạo `dev` từ nhánh mặc định hiện tại của remote và push thiết lập upstream, không đưa thay đổi đang dở vào nhánh gốc. Không sửa trực tiếp trên `dev`, `main` hoặc `master`, không ghi đè hay loại bỏ thay đổi có sẵn của người dùng.
...
- Trước khi chuyển sang chức năng khác, commit và push tiến độ của nhánh hiện tại. Tạo nhánh mới từ `dev` đã cập nhật; nếu chức năng phụ thuộc vào nhánh chưa merge, có thể tạo nhánh nối tiếp từ nhánh đó và ghi rõ nhánh gốc cùng quan hệ phụ thuộc. Không mang thay đổi chưa commit của chức năng trước sang nhánh chức năng mới; nếu working tree chứa thay đổi không liên quan, giữ nguyên và dùng worktree riêng khi cần.
- Trong lúc đang thực hiện nhiệm vụ, cứ khoảng **30 phút** kể từ lúc bắt đầu hoặc lần lưu tiến độ gần nhất, commit phần đã làm trên nhánh riêng và push nhánh đó, sau đó tích hợp phần đủ điều kiện vào `dev` và push `dev`; không đợi hoàn thành toàn bộ prompt. Nhánh được tạo theo chức năng/lỗi/cập nhật ngay từ đầu, không tạo nhánh chỉ theo mốc thời gian nếu vẫn đang làm cùng một phần việc. Nếu đang chạy một thao tác chưa thể ngắt an toàn, thực hiện ngay khi thao tác đó kết thúc. Quy tắc này chỉ hoạt động khi agent đang làm việc, không phải lịch chạy nền khi phiên đã dừng.
- Trước khi đưa vào `dev`, fetch trạng thái mới nhất, tích hợp vào `dev` bằng merge thông thường và chạy các kiểm tra phù hợp trên kết quả tích hợp trước khi push. Chỉ merge phần hoạt động độc lập, không làm hỏng luồng hiện có và đã qua kiểm tra; một chức năng lớn có thể chia thành các phần nhỏ đáp ứng điều kiện này. Phần còn dở hoặc kiểm tra thất bại vẫn phải được lưu trên nhánh riêng ở mốc 30 phút, nhưng chưa merge vào `dev`; báo rõ lý do. Không push nhánh chức năng thẳng đè lên `dev` bằng refspec hoặc force-push. Nếu remote `dev` thay đổi trong lúc làm, cập nhật và kiểm tra lại phần bị ảnh hưởng trước khi thử push tiếp.
- Nếu `dev` được bảo vệ và bắt buộc PR, giữ phần đã push trên nhánh riêng và báo cần PR theo quy định repository; không tìm cách vượt branch protection. Sau mỗi lần tích hợp, tiếp tục công việc trên nhánh riêng, không sửa tiếp trên `dev`.
...
- Khi hoàn thành nhiệm vụ, commit và push phần thay đổi còn lại, tích hợp phần đủ điều kiện vào `dev` rồi push `dev` dù chưa đủ 30 phút. Báo tên nhánh công việc, commit gần nhất, kết quả push nhánh riêng và `dev`, cùng kiểm tra đã chạy. Nếu push hoặc merge bị chặn bởi quyền, xác thực, mạng hay xung đột chưa thể giải quyết chắc chắn, báo rõ nguyên nhân và giữ tiến độ đã lưu; không tự force-push hay khẳng định đã đồng bộ thành công.
