...
test('login, dashboard and dialog have no automated WCAG AA violations', async ({ page }) => {
  ...
  result = await new AxeBuilder({ page })
    ...
    .analyze();
  expect(result.violations).toEqual([]);
  await page.goto('/caregiver/contacts');
  ...
  await expect(page.getByRole('button', { name: 'Thêm mới' })).toBeFocused();
});
test('registration, password recovery and session logout work without reload', async ({ page }) => {
  const errors: string[] = [];
  page.on('pageerror', (e) => errors.push(e.message));
  await page.goto('/auth/register');
  await page.getByLabel('Địa chỉ email').fill('family-new@demo.vn');
  await page.getByLabel('Họ và tên').fill('Gia đình mới');
  ...
  await page.getByLabel('Địa chỉ email').fill('family-new@demo.vn');
  await page.getByLabel('Mật khẩu *', { exact: true }).fill('Password@2');
  await page.getByRole('button', { name: 'Đăng nhập', exact: true }).click();
  await expect(page.getByRole('heading', { name: /Xin chào/ })).toBeVisible();
